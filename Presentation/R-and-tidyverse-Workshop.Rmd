---
title: "Getting Started in R and tidyverse"
...
<div class="rtidyverse-logo-left-bottom"></div> 

# Getting Started in R and tidyverse
...
### For example, tidyverse is an R package
...
<div class="dplyr-logo-right-top"></div>

# Import the data: `read_csv()`

### First load `tidyverse` using the `library()` function
...
```{r glimpseData, highlight.output=c(1:5)}
dplyr::glimpse(cw)
```
...
# `dplyr::mutate()`: Add New Column 

.pull-left[
```{r newVar, highlight.output=c(1)}
mutate(cw, weightKg := weight/1000) #<<
cw
```
...
--
.pull-right[
```{r newVarKeep, highlight.output=c(1)}
cwM1 <- mutate(cw, weightKg = weight/1000) #<<
cwM1
```

### To save assign `assign` to object.
]
---
<div class="dplyr-logo-right-top"></div>

# `dplyr::mutate()`: Add Multiple Columns



```{r newVars, highlight.output=c(1)}
cwM2 <- mutate(cwM1, Hours=24*Time, #<<
               LongD=paste0("Diet ", Diet)) #<<
cwM2
```
...
# `dplyr::select()`: Drop Columns

.pull-left[
```{r dropCol, highlight.output=c(1)}
# Drop One Column #<<
select(cwM1, -weightKg)
```
 ]
--
.pull-right[
```{r dropCols, highlight.output=c(1), eval=FALSE}
# Drop Multiple Columns #<<
select(cw, -Diet, -Hours)
cw
```
  ]
...
--
.pull-right[
```{r renameVars, highlight.output=c(1), eval=FALSE}
# Rename: weight to Weight & LongD to Group
rename(cw, Weight = weight, Group=LongD)
cw
```
]
...
# `dplyr::arrange(`: Column Order

.pull-left[

```{r reorderVar1, highlight.output=c(1), eval=FALSE}
select(cw, Weight, Time, Group, Chick)
```
 ]
--
.pull-right[
...
```{r sumstat2, highlight.output=c(1), echo=2:3, eval=FALSE}
#options(digits = 2)
...
```{r chaining, echo=3:6, eval=FALSE}
...
```{r prettySum, echo = FALSE, eval=FALSE}
